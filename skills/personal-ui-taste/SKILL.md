...
- 涉及图表、tooltip、筛选、状态历史、响应式或交互回归时，按需读 [patterns.md](patterns.md)。
...
| T05 | 图表 | 悬浮轻微高亮/放大；点击单个堆叠色块聚焦，其他块及 tooltip 条目同步变淡 | 整列命中掩盖单块；提示与图上焦点不同步；过度动画 |
| T06 | 信息可读性 | 右侧最新数据优先好读，tooltip 自适应位置、按内容宽度展开 | 贴边把名称挤成多行、拆单词；最新数据反而最难读 |
