...
- **Preflight even when cheap:** the same pass as a real run, one look — environment active, and a live `nvidia-smi` read that picks a free card and passes it explicitly (`CUDA_VISIBLE_DEVICES=…`, never the default). A probe that lands on someone else's card wastes their run and yours.
