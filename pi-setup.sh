...
# ---- 5.2 Pi 桥扩展(抓取 Pi 会话活动 → claude-mem worker;提供 claude_mem_search)----
if [ "$PI_OK" -eq 1 ]; then
  deploy_file "$RAW/pi/extensions/claude-mem.ts" "$AGENT_DIR/extensions/claude-mem.ts" "claude-mem Pi 桥扩展" || true
  # ---- 6. subagent 定义 ----
