...
| context-mode fork | 上游 `context-mode` 与 `magic-context` **都注册 `ctx_search`** —— Pi 的同名工具检测会让启动直接 `process.exit(1)`。所以本仓库带一份本地 fork:构建后把 11 个工具改名 `ctxm_*`,另做三项口径修正。`pi-setup.sh` 会按需 clone → 跑 `context-mode/setup.sh`(同步上游 + 三层改动 + 构建 + 14 项体检)→ `pi install <本地路径>`(容器改动只需重启 pi)。**不要装上游 `npm:context-mode`**,脚本检测到会提示卸载。上游发新版后跑一次 `context-mode/setup.sh` 同步(用法/排错见该目录里的脚本注释与 `--verify`);clone 位置用 `CONTEXT_MODE_DIR` 覆盖 |
