...
| 运行默认值 | `modelThinkingLevels`：`codex-newapi/gpt-6-astra`、`codex-newapi/gpt-6-sol` 用 `xhigh`，其余全局 `high`；agent 级 `retry`：`maxRetries:8`、`baseDelayMs:4000`（4/8/16/32/64/128/256/512 秒，累计 17 分钟，加上首发最多 9 次请求），原生指数退避，不开 provider 内层重试 |
| 自动命名 | `pi-autoname@0.6.8` + `~/.pi/agent/pi-autoname.json`：`codex-newapi/gpt-6-sol` 在任务结束后命名；冷却 1440 分钟、尊重手工名称、`/autoname` 手动触发。会把最近对话片段发给命名模型；失败回退当前会话模型/文本提取。该版本配置路径固定 `~/.pi/agent`，自定义 `PI_CODING_AGENT_DIR` 时跳过此插件 |
