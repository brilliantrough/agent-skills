...
function buildItems(
	...
	for (const entry of config.segmentLayout) {
		...
		if (segment === "metrics") {
			...
			const promptTotal = metrics.input + metrics.cacheRead + metrics.cacheWrite;
			const sessionHit = promptTotal > 0 ? metrics.cacheRead / promptTotal * 100 : undefined;
			const cacheHit = [
				metric("HΣ", percentValue(sessionHit, 1), palette, "cache"),
				metric("H₁", percentValue(metrics.cacheHitPercent, 1), palette, "cache"),
			].join(" ");
