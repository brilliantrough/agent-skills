	...
	return {
			...
			try {
				...
				if (previous) {
					try {
						...
						// Rollback is best-effort; report the original model error below.
					}
				}
		...
		setThinkingLevel(level: Parameters<ExtensionAPI["setThinkingLevel"]>[0]): void {
			...
			try {
				...
				try {
					...
					// Rollback failure must not mask the original thinking-level error.
				}
				notify(
		...
		setTools(names: string[]): void {
			...
			try {
				...
				try {
					...
					// Rollback failure must not mask the original tool-selection error.
				}
		...
		async saveDisplayDefaults(): Promise<void> {
			...
			try {
				...
				if (!isActive()) return;
				notify("Agent Skills UI display defaults saved", "info");
			} catch (error) {
				if (!isActive()) return;
				notify(
		...
		const hostPromise = ctx.ui.custom<void>(
			(tui, theme, _keys, done) => {
				...
				const ensureActive = (): void => {
					if (!isOverlayLifetimeActive(lifetime)) throw new Error("Agent Skills UI is not active in this session");
				};
				if (!isOverlayLifetimeActive(lifetime)) finish();
				const component = createLifecycleOverlayComponent(
					lifetime,
...
export async function openAtelierControlCenter(
	...
	const lifetime = options.lifetime;
	if (ctx.mode !== "tui") {
		ctx.ui.notify("Agent Skills UI Control Center requires TUI mode", "warning");
		return;
	}
	if (!isOverlayLifetimeActive(lifetime)) return;
