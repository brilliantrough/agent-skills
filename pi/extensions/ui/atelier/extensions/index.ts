...
export default function atelierExtension(
	...
	const lifecycleGuardedSavePatch =
		...
		async (path, patch) => {
			if (activeSession !== targetSession) throw new Error("Agent Skills UI is not active in this session");
	...
	async function setSidebarToolNames(
		...
		try {
			await lifecycleGuardedSavePatch(targetSession)(join(getAgentDir(), "agent-skills-ui.json"), {
	...
	async function openMenu(ctx: ExtensionContext): Promise<void> {
		...
		if (!current) {
			ctx.ui.notify("Agent Skills UI is not active in this session", "warning");
			return;
		}
		...
		await openAtelierControlCenter(
			...
			ctx,
			targetRuntime,
			join(getAgentDir(), "agent-skills-ui.json"),
			{
	...
	async function openDisplay(ctx: ExtensionContext): Promise<void> {
		if (ctx.mode !== "tui") {
			ctx.ui.notify("Agent Skills UI Display settings require TUI mode", "warning");
			return;
		}
		const current = getActiveSession(ctx);
		if (!current) {
	...
	function installFooter(targetSession: ActiveSession): void {
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			const component = createFooterComponent({
				...
				// SAFETY: Pi Theme exposes the fg/bg/bold methods consumed by ThemeLike.
	...
	pi.registerCommand("ui", {
		description: "Open or control the Agent Skills UI status menu",
		handler: async (args, ctx) => {
			const parts = args.trim().toLowerCase().split(/\s+/).filter(Boolean);
			const [action, sidebarAction, ...extra] = parts;
			if (action === "display") {
				if (sidebarAction !== undefined || extra.length > 0) {
					ctx.ui.notify("Usage: /ui display", "warning");
					return;
				}
			...
			if (action === "sidebar") {
				if (ctx.mode !== "tui") {
					ctx.ui.notify("Agent Skills UI sidebar requires TUI mode", "warning");
					return;
				}
				const current = getActiveSession(ctx);
				if (!current) {
				...
				if (sidebarAction === "tools") {
					...
					if (
						...
						ctx.ui.notify("Usage: /ui sidebar tools [on|off]", "warning");
						return;
					}
				...
				if (
					...
					ctx.ui.notify("Usage: /ui sidebar [on|off]", "warning");
					return;
				}
			...
			if (action === "disable") {
				...
				ctx.ui.notify("Agent Skills UI disabled", "info");
				return;
			}
			if (action === "enable") {
				const current = getActiveSession(ctx);
				...
				ctx.ui.notify("Agent Skills UI enabled", "info");
				return;
			}
			await openMenu(ctx);
		},
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			const userPath = join(getAgentDir(), "agent-skills-ui.json");
			const projectPath = join(initializationContext.cwd, CONFIG_DIR_NAME, "agent-skills-ui.json");
			...
			localSidebar = createSidebarController({
				...
				getSnapshot: () => {
					...
					if (!current || current.token !== initializationToken)
						throw new Error("Agent Skills UI session is not published");
			...
			if (isFresh() && !shortcutRegistered) {
				try {
					pi.registerShortcut(loaded.config.shortcut as KeyId, {
						description: "Open Agent Skills UI",
			...
			if (isFresh() && !resizeShortcutRegistered) {
				pi.registerShortcut("ctrl+shift+r" as KeyId, {
					description: "Resize Agent Skills UI sidebar",
					handler: (shortcutContext) => {
						const current = getActiveSession(shortcutContext);
						if (!current?.sidebar.isVisible()) {
							shortcutContext.ui.notify("Show the Agent Skills UI sidebar before resizing it", "warning");
							return;
						}
			...
			if (!publishedSession) {
				...
				if (!isFresh()) return;
				initializationContext.ui.notify(
					`Agent Skills UI could not start: ${error instanceof Error ? error.message : String(error)}`,
					"error",
				);
				return;
			}
