...
export const configPath = join(getAgentDir(), "agent-skills-editor.json");
	...
	try {
		...
		if (file !== undefined) {
			try {
				...
				// Best-effort close: preserve the original write failure below.
			}
		}
		try {
			unlinkSync(tempPath);
...
type ConfigValue = string | number | boolean | null | ConfigRecord | unknown[] | undefined;
...
function resolvedValue(
	...
	if (hasOwn(canonical, canonicalKey)) return configValue(canonical[canonicalKey]);
	if (legacy && hasOwn(legacy, legacyKey)) return configValue(legacy[legacyKey]);
	return undefined;
}
...
function overlayKnown(raw: unknown, known: unknown): ConfigValue {
	if (!isRecord(known)) return configValue(known);
