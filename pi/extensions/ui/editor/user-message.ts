				...
				...(transform
					? {
							transform: (input: string, width: number) =>
								sanitizeUserMessageSourceText(
									transform.call(options.value, input, width),
								),
						}
					: {}),
			};
...
function nativeMarkdown(instance: PatchableUserMessagePrototype):
	...
	// SAFETY: isNativeMarkdown verified the renderer object and its native methods.
	...
	return {
		// SAFETY: native constructor/methods, text, padding and theme shape were checked above.
		renderer: renderer as unknown as Markdown,
		text: child.text,
		input: {
			// SAFETY: every required Markdown theme callback was checked in the loop above.
...
function sanitizePredecessorRender(result: string[] | void): string[] | void {
	return result ? sanitizeRenderedUserMessageLines(result) : result;
}
			...
			({ predecessor, receiver, args }) => {
				const renderPredecessor = () =>
					withSanitizedMarkdownSources(receiver, (adapted) => {
						const result = sanitizePredecessorRender(predecessor.apply(receiver, args));
						return adapted &&
