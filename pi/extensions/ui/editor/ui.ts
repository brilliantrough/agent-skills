...
export function renderWithAutocompleteCapture<T>(
	...
	let predecessor: (width: number) => string[];
	try {
		...
		list = candidate;
		predecessor = current as (width: number) => string[];
	} catch {
		return { value: render() };
	}

	const capture: AutocompleteCapture = { compatible: true, called: 0, rows: [] };
	const wrapper = function (this: AutocompleteListInternals, ...args: [number]): string[] {
		const result = predecessor.apply(this, args);
		capture.called++;
